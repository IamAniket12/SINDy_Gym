...
class LunarLander(gym.Env, EzPickle):
    ...
    def reset(
        ...
        *,
        ...
        if self.render_mode == "human":
            self.render()
        return self.step(np.array([0, 0]) if self.continuous else 1)[0], {}
    ...
    def step(self, action):
        ...
        if (self.continuous and action[0] > 0.0) or (
            ...
            x_pos = (self.lander.position.x - VIEWPORT_W / SCALE / 2) / (
                VIEWPORT_W / SCALE / 2
            )
            y_pos = (self.lander.position.y - (self.helipad_y + LEG_DOWN / SCALE)) / (
                VIEWPORT_H / SCALE / 2
            )
            x_vel = self.lander.linearVelocity.x * (VIEWPORT_W / SCALE / 2) / FPS
            y_vel = self.lander.linearVelocity.y * (VIEWPORT_H / SCALE / 2) / FPS
            linear_angle = self.lander.angle
            angular_velocity = 20.0 * self.lander.angularVelocity / FPS

            sindy_input = np.array(
                [[x_pos, y_pos, x_vel, y_vel, linear_angle, angular_velocity]]
            )
        ...
        sindy_output = self.sindy_model.predict(sindy_input, np.array([action]))
        state = [
            sindy_output[0][0],
            sindy_output[0][1],
            sindy_output[0][2],
...
register(
    id="LunarLander-Sindy-v0",
    entry_point="src.environment.lunar_lander:LunarLander",
